...
# 分离路由：智能分析/报告导出等（避免 core/blueprint.py 过长）
from core.routes.analyze import register_analyze_routes  # noqa: E402
...
# 分离模块的路由注册需要依赖本文件内定义的工具函数（如 `_public_site_base_url`），
# 因此注册放到本文件靠后位置执行。
...
# 注册拆分出去的路由模块（保持 app.py 与 URL 不变）
# 注意：必须在 app.register_blueprint(...) 之前完成注册（app.py 在 import 完成后注册）。
register_analyze_routes(
    ...
    TaskShare=TaskShare,
    decrypt_ai_config_inplace=decrypt_ai_config_inplace,
    call_ai_model=call_ai_model,
    get_chat_server_model_light=get_chat_server_model_light,
    generate_analysis_prompt=generate_analysis_prompt,
    read_file_content=read_file_content,
    perform_analysis_with_custom_prompt=perform_analysis_with_custom_prompt,
    save_analysis_report=save_analysis_report,
    generate_report_image=generate_report_image,
    build_report_html=build_report_html,
    _to_user_friendly_ai_error=_to_user_friendly_ai_error,
    _public_site_base_url=_public_site_base_url,
    _static_uploads_dir=_static_uploads_dir,
    progress_lock=progress_lock,
    analysis_progress=analysis_progress,
    SUPPORTED_AI_MODELS=SUPPORTED_AI_MODELS,
