...
本项目目前处于**大规模开源发布前的内测/快速迭代阶段**。如果你在校园网环境中部署后遇到异常（登录/提交/邮件/迁移/性能等），请优先按以下方式反馈，我们会尽快定位并在下一版修复：

- **首选反馈方式**：在本仓库提交 Issue（附：报错堆栈、`.env` 关键配置项名、复现步骤、系统版本/数据库版本）。
- **紧急联络**：邮件联系维护者（`wzlinmiaoyan@163.com`），标题建议包含「QuickForm 校园版」与学校名/单位名。

为降低影响，我们建议部署方保留“可回滚”的基本手段：

- 发布前先打一个标签或留存上一个可用版本的 commit/hash，异常时可快速回退。
- `.env` 与数据库账号密码请自行妥善保管；不要把真实密钥提交到仓库。
